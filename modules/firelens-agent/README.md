...
| `image` defaults to an `init` tag, deliberately unpinned | The one exception to this org's usual pinned-version convention — `init` tags don't share `:stable`'s version numbering, so a guessed `init-X.Y.Z` may not exist. Pin once a specific tag is verified working. |
